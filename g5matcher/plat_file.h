 ...
 *
 */
int plat_save_raw_image(char* path, unsigned char* pImage, unsigned int width, unsigned int height);
 ...
 *
 */
int plat_load_raw_image(char* path, unsigned char* pImage, unsigned int width, unsigned int height);
...
int plat_file_is_exist(const char* path);
