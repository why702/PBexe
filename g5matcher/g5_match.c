...
void *pb_malloc(size_t size) { return malloc(size); }
void pb_free(void *ptr) { free(ptr); }
...
static void set_image_class_type_num(void *ctx) {
  set_required_minimum_nbr_of_subtemplates_v2(ctx, FP_IMAGE_TYPE_ENROLL,
                                              g_max_enroll_count);
  set_required_minimum_nbr_of_subtemplates_v2(ctx, FP_IMAGE_TYPE_DRY,
                                              g_max_dry_count);
}
...
int algorithm_initialization() {
  ...
  int ret;
  ret =
      algorithm_initialization_v2(&g_session.g_ctx, g_decision_data,
                                  g_decision_data_len, g_session.g_sensor_type);
  // General config
  set_image_class_type_num(g_session.g_ctx);
  ret = set_algo_config_v2(g_session.g_ctx, FP_OP_ENABLE_SPD, g_session.g_spd);
  ret = set_algo_config_v2(g_session.g_ctx, FP_OP_RESOLUTION,
                           g_session.g_resolution);
  if (g_session.g_centroid_X > 0 && g_session.g_centroid_Y > 0 &&
      g_session.g_radius > 0) {
    ret = set_algo_config_v2(g_session.g_ctx, FP_OP_CENTROID_X,
                             g_session.g_centroid_X);
    ret = set_algo_config_v2(g_session.g_ctx, FP_OP_CENTROID_Y,
                             g_session.g_centroid_Y);
  ...
  if (g_session.g_dyn_mask_threshold > 0 && g_session.g_dyn_mask_radius > 0) {
    ...
    ret = set_algo_config_v2(g_session.g_ctx, FP_OP_DYN_MASK_RADIUS,
                             g_session.g_dyn_mask_radius);
  }
  // Enroll config
  ret = set_algo_config_v2(g_session.g_ctx, FP_OP_MAX_ENROLL_COUNT,
                           g_max_enroll_count);
  ...
  ret = set_algo_config_v2(g_session.g_ctx, FP_OP_SET_FIRST_N_LOWER_FAR,
                           g_first_n_lower_far);
  return ret;
}
...
void images_compare_(BYTE **raw1, BYTE **raw2, int w, int h, int *match_score,
  ...
  // extract feature
  extract_feature_v2(g_session.g_ctx, raw1[0], w, h, FP_IMAGE_TYPE_NORMAL,
                     &extract_finger_temp1, &extract_finger_temp1_size);

  extract_feature_v2(g_session.g_ctx, raw2[0], w, h, FP_IMAGE_TYPE_NORMAL,
  ...
  verify_init.enroll_temp_array[0] =
      (BYTE *)plat_alloc(extract_finger_temp1_size * sizeof(BYTE));
  ...
  // matching
  status = verify_template_v2(g_session.g_ctx, extract_finger_temp1,
...
void images_compare_by_algo(unsigned char** raw1, unsigned char** raw2, int w, int h, int* match_score, int* rot, int* dx, int* dy, struct algo_info* algo_info){
...
void images_compare_1_by_algo(BYTE** raw1, BYTE** raw2, unsigned char* mask1, unsigned char* mask2, int w, int h, int dpi, int* match_score, int* rot, int* dx, int* dy,
